...
\section{2.2 圆柱镜面反射的几何映射模型}

为系统描述纸面图案与镜面图案之间的关系，本文建立正向映射 $T$ 与逆向映射 $T^{-1}$，并在不同坐标系下给出其数学表达。全局坐标系约定如下：

\begin{itemize}
\item \textbf{3D 直角坐标系} $(x,y,z)$：用于光线追踪、反射定律推导和向量运算；
\item \textbf{镜面柱面参数坐标系} $(\theta,z)$：用于镜面离散化与镜面图案 $G(\theta,z)$ 的定义；
\item \textbf{纸面极坐标系} $(\rho,\phi)$：以圆柱轴心 $(x_0,y_0)$ 为极点，用于变形分析与对称性研究；
\item \textbf{纸面直角坐标系} $(u,v)$：以 A4 纸左下角为原点，用于最终图像生成与打印输出。
\end{itemize}
...
\subsection{2.2.1 基于镜面参数坐标 $(\theta,z)$ 的映射模型（核心）}
...
\subsection{2.2.2 基于纸面极坐标 $(\rho,\phi)$ 的映射模型}
...
\subsection{2.2.3 基于纸面直角坐标 $(u,v)$ 的映射模型}
...
\item 使用径向基函数（RBF）或网格数据插值方法，将散点转换为规则的纸面图像 $F(u,v)$；
\item 通过像素物理尺寸 $a_{\text{pix}}$（由 DPI 确定）完成从物理坐标到离散像素矩阵 $(i,j)$ 的转换。
\end{enumerate}
