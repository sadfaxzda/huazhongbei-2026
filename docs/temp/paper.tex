...
$\mathbf{P}_{mirror}$ & 镜面上点的 3D 直角坐标 & mm & $x = x_0 + R\cos\theta,$ \newline $y = y_0 + R\sin\theta$ \\
$\mathbf{n}$ & 镜面点处的外法向量 & — & $\mathbf{n} = (\cos\theta, \sin\theta, 0)$ \\
\bottomrule
\end{tabular}
\end{table}
...
\subsubsection*{使用说明}
\begin{enumerate}
    \item 符号表内部不写公式推导,只定义符号。公式推导放在 2.4 节。
    \item 全文必须严格遵守本表
    \item 书写规范:
    \begin{itemize}
        \item 向量加粗:$\mathbf{E}, \mathbf{n}, \mathbf{d}, \mathbf{r}, \mathbf{P}$
        \item 标量不加粗:$R, H, \theta, z$ 等
        \item 下标与主变量用同一字母时,用小写下标区分:$x_E$ 表示 $\mathbf{E}$ 的 x 分量
        \item 映射与算子用大写字母:$T, T^{-1}, F, G$
    \end{itemize}
\end{enumerate}
...
\[
\mathbf{P}_{\mathrm{mirror}}=\bigl(x_0+R\cos\theta,\;y_0+R\sin\theta,\;z\bigr)
\]
...
镜面上的点参数化为：
\[
\mathbf{P}_{\mathrm{mirror}}(\theta,z)=\bigl(x_0+R\cos\theta,\;y_0+R\sin\theta,\;z\bigr).
\]
单位外法向量为 $\mathbf{n}(\theta)=(\cos\theta,\sin\theta,0)$。

逆向映射（由镜面到纸面）是本文核心算子。从观察者 $\mathbf{E}$ 出发，经镜面点 $\mathbf{P}_{\mathrm{mirror}}(\theta,z)$ 反射后与纸面 $z=0$ 相交于纸面点 $\mathbf{P}_{\mathrm{paper}}=(u(\theta,z),v(\theta,z),0)$。令单位视线向量
\[
\mathbf{V}=\frac{\mathbf{E}-\mathbf{P}_{\mathrm{mirror}}}{\lVert\mathbf{E}-\mathbf{P}_{\mathrm{mirror}}\rVert},
\]
根据反射定律，反射方向向量 $\mathbf{r}$ 满足：
\[
\mathbf{r}(\theta,z)=\mathbf{V}-2\bigl(\mathbf{V}\cdot\mathbf{n}(\theta)\bigr)\,\mathbf{n}(\theta).
\]
...
\[
\mathbf{V}\cdot\mathbf{n}(\theta)<0\quad\text{（镜面正面朝外可见）},\qquad r_z<0\quad\text{（反射线向下与纸面相交）}.
\]
不满足上述条件的镜面点予以剔除，不参与图案生成。

\subsubsection{基于纸面极坐标 $(\rho,\phi)$ 的映射模型}

\textbf{对称放置假设}：约定观察者位于 $x$ 正半轴方向，即 $\mathbf{E}=(D,0,z_E)$，$\phi=0$ 方向指向观察者投影方向。
...
\begin{enumerate}
    ...
    \item 使用 \textbf{Delaunay 三角剖分线性插值}将散点转换为规则的纸面图像 $F(u,v)$；在散点稀疏或需要光滑性时可改用薄板样条（TPS）；
...
观察者眼点 $\mathbf{E}=(D,0,z_E)$。设主视线 $\hat{\mathbf{v}}_0=(-1,0,0)$ 指向圆柱轴心，虚拟视平面 $\Pi$ 垂直于主视线、距眼点 $d_0$，即 $\Pi:\;x=D-d_0$。视平面上以主视线交点为原点建立局部坐标 $(s,t)$（$s$ 水平、$t$ 垂直），则像素对应的空间点为
\[
...
\begin{enumerate}
    ...
    \item 在该散点集上对 $(\theta_k,z_l)$ 做二维 Delaunay 三角剖分线性插值，求得视平面坐标 $(s_{kl},t_{kl})$；
...
5. A4 适配与输出 & $(u,v)=(x_0+\rho\cos\phi,\;y_0+\rho\sin\phi)$；调整 $(x_0,y_0)$；以 $\geq300$ DPI 输出 & 必要时优化 $R,H$ 或裁剪 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{插值方法}：散点到网格阶段以 Delaunay 三角剖分线性插值为默认方法；在散点稀疏或需要光滑性时改用薄板样条（TPS），避免最近邻造成的马赛克。
...
\[
u=x_0+\rho\cos\phi,\qquad v=y_0+\rho\sin\phi.
\]
...
原图简化 $\rightarrow$ 参数选取(给具体 $R$、$H$、$E$ 数值)$\rightarrow$ 纸面图案结果 $\rightarrow$ 合成效果图。
