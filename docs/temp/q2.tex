...
\[
\mathbf{d}\cdot \mathbf{n}(\theta)<0 \quad\text{(镜面点位于观察者可见侧)},\qquad r_z<0 \quad\text{(反射线向下到达纸面)},
\]
使得逆向映射 $T^{-1}$ 将该镜面点映至 $(u,v)$。等价地，
\[
\mathcal{I}_{\mathrm{vis}} = T^{-1}\bigl(\{(\theta,z)\in[\theta_{\min},\theta_{\max}]\times[z_{\min},z_{\max}]: \mathbf{d}\cdot\mathbf{n}(\theta)<0,\; r_z<0\}\bigr).
\]
